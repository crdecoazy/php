...
<html>
  ...
  <body>
    <div class='container pt-5'>
      <h1 class='text-center text-primary'>SAKTHI BALA AUTO WORKS</h1><hr>
      <?php
        if(isset($_POST["submit"])){
          $invoice_no=$_POST["invoice_no"];
          $invoice_date=date("Y-m-d",strtotime($_POST["invoice_date"]));
          $cname=mysqli_real_escape_string($con,$_POST["cname"]);
          $cvechile_no=mysqli_real_escape_string($con,$_POST["cvechile_no"]);
          $ckm=mysqli_real_escape_string($con,$_POST["ckm"]);
		  $cphone=mysqli_real_escape_string($con,$_POST["cphone"]);
          $grand_total=mysqli_real_escape_string($con,$_POST["grand_total"]);
          
          $sql="insert into invoice (INVOICE_NO,INVOICE_DATE,CNAME,CVECHILE_NO,CKM,CPHONE,GRAND_TOTAL) values ('{$invoice_no}','{$invoice_date}','{$cname}','{$cvechile_no}','{$ckm}','{$cphone}','{$grand_total}') ";
          if($con->query($sql)){
            $sid=$con->insert_id;
        ...
        <div class='row'>
          ...
          <div class='col-md-8'>
            ...
            <div class='form-group'>
              ...
              <input type='text' name='cvechile_no' required class='form-control'>
            </div>
            <div class='form-group'>
              <label>KM:</label>
              <input type='text' name='ckm' required class='form-control'>
            </div>
			<div class='form-group'>
              <label>phone:</label>
